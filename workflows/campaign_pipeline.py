...
def run_pipeline(
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=2) as pool:
            ...
            try:
                video_path, video_cost, video_error = video_future.result(timeout=1800)
            except FuturesTimeout:
                video_future.cancel()
                raise RuntimeError("Video productie timeout (>30 min) — probeer opnieuw of gebruik een kortere video")
            if not video_path:
                raise RuntimeError(
